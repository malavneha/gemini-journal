...
export default function App() {
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      setAuthError(
        err?.message ||
          'Unable to sign in with Google. Please check popup permissions or network connection.'
      );
    } finally {
      setAuthLoading(false);
    }
  ...
  const handleReflectAndSave = async (promptText: string) => {
    if (!user) {
      setReflectError(
        'You must be signed in to reflect and save journal interactions.'
      );
      return;
    }
    ...
    try {
      ...
      // returns a plain-text 503/error response.
      const rawText = await response.text();

      let data: Partial<ReflectionResponse> = {};
      ...
      try {
        ...
        if (response.status === 503) {
          throw new Error(
            'Gemini is temporarily busy. Please tap Retry in a few seconds.'
          );
        }
      ...
      if (!response.ok || !data.success) {
        ...
        throw new Error(
          data.error ||
            `Failed to generate reflection from Gemini AI (HTTP ${response.status}).`
        );
      }

      if (!data.reflection) {
        throw new Error(
          'Gemini returned an empty reflection. Please tap Retry.'
        );
      }

      const newInteractionId = `journal_${Date.now()}_${Math.random()
        .toString(36)
        .slice(2, 8)}`;
      ...
      setEntries((prev) => [
        savedEntry,
        ...prev.filter((e) => e.id !== savedEntry.id),
      ]);

      setLastPrompt('');
    } catch (err: any) {
      console.error(
        'Error during journal reflection and save:',
        err
      );

      setReflectError(
        err?.message ||
          'An unexpected error occurred while reflecting on your thought.'
      );
    } finally {
    ...
    <div className="min-h-screen bg-stone-950 text-stone-100 flex flex-col font-sans selection:bg-amber-500/30 selection:text-amber-200">
            ...
            <section className="space-y-4">
              ...
              {entriesLoading && entries.length === 0 ? (
                <div className="py-12 flex flex-col items-center justify-center text-stone-400">
                  ...
                  <p className="text-xs">
                    Fetching your private journal records...
                  </p>
                </div>
              ) : entries.length === 0 ? (
                <EmptyJournal
      ...
      <footer className="w-full border-t border-stone-900 py-4 text-center text-xs text-stone-500">
        <div className="max-w-4xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          <span>
            Personal Gemini Journal &bull; Cloud Run AI Challenge Baseline
          </span>
